                ...
                <div class="detail-form">
                    ...
                    <div class="form-group">
                        ...
                        <el-input 
                            ...
                            placeholder="写下您今天的想法,感受或发生的有趣的事情..." 
                            type="textarea" 
                            :rows="5" 
                            maxlength="2000"
                            show-word-limit
...
const submit = () => {
  ...
  if (!diaryForm.moodScore) {
    ...
    const diaryContent = (diaryForm.diaryContent || '').trim()
    const emotionTriggers = (diaryForm.emotionTriggers || '').trim()
    if (!diaryContent && !emotionTriggers) {
        ElMessage.error('请至少填写日记内容或情绪触发因素')
        return
    }

    if (diaryContent && diaryContent.length < 5) {
        ElMessage.error('日记内容至少填写5个字，便于AI分析')
        return
    }
