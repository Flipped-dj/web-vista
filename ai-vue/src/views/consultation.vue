        ...
        <div class="sidebar">
            ...
            <div class="ai-assistant-info">
                <div class="breathing-circle">
                    ...
                    <h3 class="assistant-name">R&B AI助手</h3>
                    ...
                    <div v-for="session in sessionList" :key="session.id" @click="handleSessionClick(session)" class="session-item">
                        <div class="session-info">
                            ...
                            <div class="session-actions">
                                <el-button text type="danger" size="large" @click="handleDeleteSession(session.id)">
                                    <el-icon>
                                        <DeleteFilled />
                                    </el-icon>
                                </el-button>
...
const createNewFrontendSession = () => {
  ...
  currentEmotion.value = {
    ...
    emotionScore: 50,        // 情绪评分（0-100）
    isNegative: false,       // 是否为负面情绪
    riskLevel: 0,            // 风险等级（0-3）
    suggestion: '情绪状态平稳',  // 情绪建议
    improvementSuggestions: [],  // 改进建议
  }
  // 创建一个临时会话对象
  const newSession = {
    sessionId: `temp_${Date.now()}`,  // 临时会话ID，使用时间戳
    status: 'TEMP',                  // 会话状态为临时
...
// 当前会话对象
const currentSession = ref(null)  // 存储当前活跃的会话信息
const sessionList = ref([])       // 存储会话列表

// 对话消息
const messages = ref([])           // 存储当前会话的消息列表
const userMessage = ref('')        // 存储用户输入的消息
const isAITyping = ref(false)      // 标记AI是否正在输入
...
const sendMessage = () => {
  // 验证输入内容
  if (!userMessage.value.trim()) return  // 如果输入为空或仅为空格，直接返回
  ...
  if (currentSession.value.status === 'TEMP') {
    ...
    // 添加用户消息到消息列表
    messages.value.push({
        id: Date.now(),               // 消息ID，使用时间戳
        senderType: 1,                // 发送者类型：1-用户，2-AI
...
const startNewSession = (message) => {
  // 构建会话参数
  const sessionParams = {
    initialMessage: message  // 初始消息内容
  }

  // 根据当前会话状态设置会话标题
  if (currentSession.value.sessionTitle === '新对话') {
    // 新创建的临时会话，使用带时间戳的默认标题
    sessionParams.sessionTitle = `R&B AI助手 - ${new Date().toLocaleString()}`
  } else {
    // 历史会话，沿用原有标题
  ...
  startSession(sessionParams).then(res => {
    ...
    const sessionData = {
        ...
        sessionTitle: sessionParams.sessionTitle  // 会话标题
    }
...
const startAIResponse = (sessionId, userMessage) => {
  ...
  // 创建AI消息对象
  const aiMessage = {
    id: `ai_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,  // 生成唯一ID
    senderType: 2,                // 发送者类型：2-AI
    content: '',                  // 初始内容为空
...
const handleSessionClick = (session) => {
  console.log(session, 'session')  // 打印会话信息
  ...
  // 更新当前会话对象数据
  const sessionData = {
    sessionId: "session_" + session.id,  // 构建完整的会话ID
    status: 'ACTIVE',                    // 会话状态为活跃
